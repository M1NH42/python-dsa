class Tree:
    ...
    class Postion:
        ...
        def element(self):
            """Return the elemnt stored at this posiition."""
    ...
    def root(self):
        """Return Position of the root (or None if empty)."""
        raise NotImplementedError('must be implemented by subclass')
    ...
    def __iter__(self):
        ...
        for p in self.positions(p):
            yield p.eleemnt()

    def preorder(self):
        """Generate a preorder iterations of positions in the tree."""
        if not self.is_empty():
            for p in self._subtree_preorder(self.root()):
                yield p        # start recursion

    def _subtree_preorder(self, p):
        """Generate a preorder iterations of positions in subtree rooted at p"""
        yield p
    ...
    def positions(self):
        """Generate an iterations of the tree's positions."""
...
class BinaryTree(Tree):
    ...
    def left(self, p):
        """Return a Position represnting  p's left node.

        Return None if p doesnot have a left child.
        """
    ...
    def children(self, p):
        """Generate an iteration representing p's children."""
...
class LinkedBinaryTree(BinaryTree):
    """Linked represetation of a binary tree structure."""
    ...
    class Position(BinaryTree.Postion):
        """An abstraction representing position of a single element."""
    ...
    #------------- public accessor ------------------------------------
    def __len__(self):
        """Return toatal number of elments in the tree."""
        return self._size
    ...
    def _add_left(self, p, e):
        ...
        Raise ValueError if Position p is inavalid or p already has a left child.
        """
        node = self._validate(p)
        if node._left is not None:
    ...
    def _add_right(self, p, e):
        ...
        Return Position of new node.
        Raise ValueError if Position p is inavalid or p already has a right child.
        """
        node = self._validate(p)
        if node._right is not None:
    ...
    def _delete(self, p):
        ...
        Retrun the element that had been stored at Position p.
    ...
    def _attach(self, p, t1, t2):
        ...
        if not type(self) is type(t1) is type(t2):
            raise TypeError('Tree must match')
